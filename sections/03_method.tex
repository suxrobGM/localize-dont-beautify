...
We treat each editing model as a black box behind a remote API: it accepts an
...
prompts fixed. A common seed is supplied to every model; the GPT Image~2 and
...
\begin{itemize}
  ...
  \item \textbf{Masked composite.} The model still edits the whole image, but we
  ...
  \item \textbf{Masked inpaint.} For models that accept an explicit mask, we send
...
Editing APIs routinely shift, rescale, or re-crop the head, so pasting the raw
...
Relative to the metric region, we erode it off the face silhouette, clip it medial
of the ears so the seam avoids the sideburns, and extend it further down the neck
so a narrowed jawline blends into shadow or clothing. The feather is a wide
