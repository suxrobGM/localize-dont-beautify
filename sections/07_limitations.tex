...
population inference. Each cell contains one generated output, and two models
...
zone covers only part of the face, and it abstains on profile views, precisely
where rhinoplasty visualization is important; compositing guarantees off-mask
preservation, so that result verifies system behavior rather than model quality.
ArcFace's 0.6 line is heuristic. The postoperative comparison, though baselined
against each face's input photograph, lives entirely in identity-embedding
space; an edit could approximate the surgical result anatomically while losing
embedding similarity, and neither score is a validated surgical-outcome metric.
...
demographic disparities~\cite{buolamwini2018gendershades}.

Finally, provider endpoints and aliases can change without notice. Run dates and
